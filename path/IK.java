 ...
 * Description : Inverse kinematics for a robot arm.
 * Date        : June 2012
...
public class IK
{
	...
	{
		double shoulderdist = robotJoints.get("shoulder").d;
		double elbowdist = robotJoints.get("elbow").d;
		double shouldera = robotJoints.get("shoulder").a;
		
		j.zed = wristCoords.z + shoulderdist + elbowdist;

		double x = wristCoords.y;
		double y = wristCoords.x;
		double c = ( Math.pow( x, 2 ) + Math.pow( y, 2 ) - Math.pow( shouldera, 2 ) - Math
				.pow( shouldera, 2 ) ) / ( 2 * Math.pow( shouldera, 2 ) );
		double s = Math.sqrt( 1 - Math.pow( c, 2 ) );

		double shoulder = Math.toDegrees( Math.atan2( y, x )
				- Math.atan2( shouldera * s, shouldera + shouldera * c ) );
		double elbow = Math.toDegrees( Math.atan2( s, c ) );

		if ( shoulder < -90 )
		{
			j.shoulder = Math.toDegrees( Math.atan2( y, x )
					- Math.atan2( 253.5 * -s, shouldera + shouldera * c ) );
